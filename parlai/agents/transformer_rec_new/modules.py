...
from parlai.agents.dkcrs.modules import DKCRS
...
class TransformerMemNetModel(TorchGeneratorModel):
    ...
    def __init__(self, opt, dictionary, abstract_vec, abstract_mask, kg_mask, dialogue_mask):
        ...
        self.recommend_model = DKCRS(opt["n_entity"], 0, opt['dim'], 0, kg)
        state_dict = torch.load('saved_128d_rgcn_none_l2_no_preference_17r_2/dkcrs')['model']
        # state_dict = torch.load('../parlai/tasks/crs/saved_128d_rgcn_none_l2_no_preference_17r_2/dkcrs')['model']
...
class TransformerCopyNetModel(TorchGeneratorModel):
    ...
    def __init__(self, opt, dictionary, kg_mask, dialogue_mask, kg_mapping):
        ...
        opt["n_entity"] = len(set(entity2entity_id.values()))
        self.recommend_model = DKCRS(opt["n_entity"], 16, opt['dim'], 0, kg, num_bases=8, return_all=True)
        state_dict = torch.load('kecrs/dkcrs')['model']
        # state_dict = torch.load('../parlai/tasks/crs/kecrs/dkcrs')['model']
        self.recommend_model.load_state_dict(state_dict)
...
class TransformerGeneratorModel(TorchGeneratorModel):
    def __init__(self, opt, dictionary):
        ...
        state_dict = torch.load('saved2/kbrd')['model']
    ...
    def output(self, tensor):
        ...
        count = output[0].detach().cpu().numpy()
        sum_c = sum(count)
        if hasattr(self, 'user_representation'):
            ...
            count2 = up_bias[0].detach().cpu().numpy()
            sum_c2 = sum(count2)
            output += up_bias
        return output
