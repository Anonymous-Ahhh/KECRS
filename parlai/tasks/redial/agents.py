        ...
        for instance in self.instances:
            ...
            for movie_id in initiator_ques:
                ...
                if initiator_ques[movie_id]["liked"] == 1:
                    liked_movie.append(self.entity2entityId[entity])
                elif initiator_ques[movie_id]["liked"] == 0:
                    disliked_movie.append(self.entity2entityId[entity])
            message_idx = 0
            new_episode = True
            ...
            while message_idx < len(messages):
                ...
                if source_text != [] or target_text != []:
                    ...
                    if len(target_text) == 0:
                        continue
                    # for source_movie in source_movie_list + previously_mentioned_movies_list:
                    #     if source_movie in target_movie_list:
                    #         target_movie_list.remove(source_movie)
                    # for target_movie in target_movie_list:
                    #     if target_movie in disliked_movie:
                    #         target_movie_list.remove(target_movie)
                    yield (source_text, [target_text], None,
                           [str(turn), ' '.join(previously_mentioned_movies_list + source_movie_list),
                            ...
                            ' '.join(list(map(str, liked_movie))), ' '.join(list(map(str, disliked_movie)))
                            ], None), new_episode
                    new_episode = False
